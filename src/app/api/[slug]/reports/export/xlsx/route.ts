/**
 * GET /api/[slug]/reports/export/xlsx — Export attendance report as Excel
 *
 * Query params:
...
export async function GET(
  ...
  try {
    ...
    // Export to Excel
    const buffer = await exportToExcel(records, tenant);

    // Build filename
    const filename = `laporan-absensi-${slug}-${parsed.data.startDate}-${parsed.data.endDate}.xlsx`;
